...
- reconstructs the spectral convolution kernel `K` from the mean autocorrelation `<A_F>` as `K = IFT{+/- sqrt[FT(<A_F>)]}`, where `FT` and `IFT` are the Fourier transform operation and its iverse, respectively. The calculation of `K` includes an algorithm for the sign disambiguation of the `IFT` argument, and one to minimise the impact of artefacts or real signal in the datacube (`Z` in the image below);
