...
Python script to estimate the spectral resolution of a data cube from its noise properties (paper in preparation).
...
- reconstructs the spectral convolution kernel `K` (normalised to unit peak) from the mean autocorrelation `<A_F>` as `K = IFT{+/- sqrt[FT(<A_F>)]}`, where `FT` and `IFT` are the Fourier transform operation and its inverse, respectively. The calculation of `K` includes an algorithm for the sign disambiguation of the `IFT` argument, and one to minimise the impact of artefacts or real signal (`Z` in the image below);
- calculates the spectral resolution defined as the area of the spectral convolution kernel `K`;
...
The output image for the cube available [here](https://gitlab.com/SoFiA-Admin/SoFiA-2/-/wikis/home) looks something like this:

![Image](sofiatestcube.png)
